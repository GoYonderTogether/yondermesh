...
description: 一个自托管的 Agent 上下文总线，让你的 AI 编程 Agent 之间共享上下文、互相查询、跨设备跨 CLI 交接任务。
outline: [2, 3]
---
...
暴露一个 MCP（Model Context Protocol）服务器供任何支持 MCP 的 Agent 查询。结果是：你的
Agent 不再是孤岛，而是跨设备、跨 CLI 共享同一个工作面。

## 问题：Agent 是孤岛

如果你在多台机器上使用多个 AI 编程 Agent——Claude Code、Codex、Aider、Gemini CLI、
Cursor、Windsurf、Trae、Continue、OpenCode——你多半已经体会过这些痛点：
...
每台设备都是隔离的。最终结果是碎片化、有损、且作为一个整体根本无法查询。
...
自动从每台设备上的所有 CLI Agent 采集 session 到本地 SQLite。yondermesh 内置了多个
...
并拉取对端更新后在本地解密。relay 是一根"哑管道"：它永远不持有解密密钥。
...
Agent A 可以从 Agent B 停下的地方继续，即使在不同机器上。`ymesh handoff <session-id>`
命令会构建一个浓缩的 `HandoffPackage`——摘要加上近期工具调用加上任务计划——可以喂给
另一个 Agent 的上下文窗口。这就是把孤立 session 变成跨设备连续工作流的桥梁。
...
- [安装](/zh/guide/installation)——npm 安装、从源码构建、release 管理以及 macOS
  LaunchAgent 服务。
- [架构](/zh/guide/architecture)——三个平面（local / sync / mount）、代码地图、以及保持
  它们干净的不变量。
