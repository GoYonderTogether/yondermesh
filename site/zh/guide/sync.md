---
title: 跨设备同步
description: yondermesh 将通过自托管 relay 在你的设备之间同步 session，离开设备的永远是密文。介绍其工作原理、配置方式与隐私模型。
outline: [2, 3]
---

# 跨设备同步

::: warning planned —— 尚未实现
跨设备同步是**规划中**的能力。下面的设计是真实的，但 `src/sync/agent.ts` 目前是 TODO 空壳——尚未同步任何数据。本页描述的是设计架构，不是已发布的行为。
:::

你的 AI 编码代理分布在多台机器上——一台笔记本、一台台式机、一台工作虚拟机。上下文应该跟随。yondermesh 设计用于通过自托管 relay 在你的设备之间同步 session，离开设备的永远是密文。
