...
The output is the `ActiveSummary` shape from `src/store/types.ts`: a count of active sessions, a count of live (currently-writing) sessions, breakdowns by source, and a per-session list ordered by `lastSeenAt` descending. This is the same data that powers the MCP tool `list_active`.
...
- **`ymesh handoff <id>`** builds a `HandoffPackage` — compacted summaries plus recent messages plus a task plan — for a single session, so agent B can pick up where agent A left off. The same builder backs the `handoff` MCP tool. See `src/mcp/codex-handoff.ts`.
...
- [MCP Server](/guide/mcp) — the MCP tools (`search_sessions`, `list_active`, `handoff`, `get_session`, …) that expose sessions to agents.
