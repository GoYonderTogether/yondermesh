...
> **Agent 时代的第一个协作中枢。** 一个 daemon，一个 MCP server，零侵入 —— 把所有设备、所有 CLI 的 Agent 聚合成一个有共享工作面的整体。
...
- **采集（Collect）** —— 把每台设备上每个 CLI 的 session 全部收进一个本地 SQLite。你的 Agent 不再是孤岛，而是聚合成一个有共享工作面的整体。
- **同步（Sync）** —— 通过自托管 relay 做端到端加密跨设备同步。离开你机器的永远是密文。
- **查询（Query）** —— 任何 agent 通过 MCP 工具查询其他 agent 的上下文。拓扑感知、来源感知、项目感知。
- **接力（Hand off）** —— Agent A 从 Agent B 停下的地方继续，即使换了机器。session 不再在边界处消亡，而是成为一条连续的工作流。
- **同步注入（Send）** —— 向任意已接入的 CLI agent 实时发一条 user message 并同步拿到回复。28 个 CLI，6 种通道（cli-spawn / stdin / http-api / ws-rpc / tmux / applescript），3 种模式（停止 / 运行中 / 新建）。即使对方 agent 没配 model，至少也能返回错误消息，而不是沉默。
...
- **同步注入（Mailbox v3）** —— `ymesh send` / `yondermesh_send` 把 user message 投递到任意已接入的 CLI 并返回清洗后的回复。6 种触发通道，3 种模式（停止 / 运行中 / 新建，新建模式可选 `model` + `effort`）。失败永不沉默：未知 CLI、未配 model、上游 API 限流，都会以文本形式回到 response 里。
...
| **触发** | MailboxCore → TriggerAdapter（cli-spawn / stdin / http-api / ws-rpc / tmux / applescript）→ 目标 CLI → ReplyAdapter → 审计日志 |

完整代码地图和架构不变式见 [ARCHITECTURE.md](ARCHITECTURE.md)。

## CLI 覆盖

yondermesh 读取每个 CLI agent 的原生 session 格式，并通过触发层向 28 个已支持的 CLI 中的任意一个同步注入消息。各 CLI 的挂载策略：
...
完整适配器矩阵（28 个 CLI，覆盖等级 A/B/C）：[文档/reference/adapters](https://goyondertogether.github.io/yondermesh/zh/reference/adapters)
...
- [x] **M2** —— `handoff_task`（agent 间任务委派，跨设备）
- [x] **Mailbox v3** —— 同步注入（`ymesh send` / `yondermesh_send`）；28 个 CLI，6 种触发通道，3 种模式
